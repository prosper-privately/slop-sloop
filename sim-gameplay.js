(function (globalScope) {
  ...
  function sailForces({
    ...
    const windSlotEfficiency = 0.22 + 0.78 * Math.sin(Math.min(Math.PI, absWindAngle));
  ...
  function computeSailingStep({
    ...
    let trimEfficiency = Math.max(0, Math.min(1, 1 - (trimError / config.trimWindow) * closeHauledPenalty));
    const inIrons = absWindAngle < config.noGoAngle;
    if (!inIrons) {
      trimEfficiency = Math.max(trimEfficiency, config.minTrimEfficiency);
    }
  ...
  function launchReachHeading(trueWindDir, tackSign = 1, reachAngle = Math.PI * 0.34) {
    const sign = tackSign >= 0 ? 1 : -1;
    return normalizeSigned(trueWindDir + sign * reachAngle);
  }
