...
test('default trim still yields some drive on common points of sail', () => {
  const angles = [0.75, 1.0, 1.3, 1.7, 2.2, 2.7];
  const surges = angles.map((absWindAngle) => computeSailingStep({
    heading: 0,
    sail: 0.55,
    speed: 0,
    apparentWindDir: absWindAngle,
    apparentWindSpeed: 11,
    absWindAngle,
    rudder: 0
  }).surge);

  const movingCases = surges.filter((s) => s > 0.05).length;
  assert.ok(movingCases >= 5, `insufficient moving cases at default trim: ${surges.join(', ')}`);
});

test('boat accelerates from spawn-like defaults without immediate trim input', () => {
  let heading = 0;
  let sail = 0.55;
  let vx = 0;
  let vy = 0;
  const absWindAngle = 1.25;

  for (let i = 0; i < 80; i++) {
    const speed = Math.hypot(vx, vy);
    const apparentWindDir = heading + absWindAngle;
    const { motion } = computeAndApplySailingStep({
      heading,
      sail,
      speed,
      apparentWindDir,
  ...
  assert.ok(Math.hypot(vx, vy) > 1.0, `spawn-like boat failed to accelerate: ${Math.hypot(vx, vy)}`);
});


test('launchReachHeading starts on a legal point of sail (not in irons)', () => {
  const wind = 0.8;
  const starboard = launchReachHeading(wind, 1);
  const port = launchReachHeading(wind, -1);
  const starboardRel = Math.abs(Math.atan2(Math.sin(wind - starboard), Math.cos(wind - starboard)));
  const portRel = Math.abs(Math.atan2(Math.sin(wind - port), Math.cos(wind - port)));

  assert.ok(starboardRel > DEFAULT_NO_GO_ANGLE + 0.2);
  assert.ok(portRel > DEFAULT_NO_GO_ANGLE + 0.2);
});

test('spawn heading generated by launchReachHeading accelerates without controls', () => {
  const wind = 1.1;
  let heading = launchReachHeading(wind, 1);
  let sail = 0.55;
  let vx = 0;
  let vy = 0;

  for (let i = 0; i < 70; i++) {
    const apparentWindDir = wind;
    const trueWindRel = Math.atan2(Math.sin(apparentWindDir - heading), Math.cos(apparentWindDir - heading));
    const absWindAngle = Math.abs(trueWindRel);
    const speed = Math.hypot(vx, vy);
    const { motion } = computeAndApplySailingStep({
      heading,
      sail,
  ...
  assert.ok(Math.hypot(vx, vy) > 0.9, `launch heading failed to accelerate: ${Math.hypot(vx, vy)}`);
});
